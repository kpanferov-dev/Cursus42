...
class GraphVisualizer:
    ...
    def render_static(self, output_path: Optional[str] = None) -> None:
        ...
        legend_items = [
            ...
            mpatches.Patch(color=ZONE_COLORS[ZoneType.NORMAL.value], label="Normal"),
            mpatches.Patch(
                color=ZONE_COLORS[ZoneType.RESTRICTED.value], label="Restricted (2T)"
            ),
            mpatches.Patch(
    ...
    def render_with_drones(
        ...
        for zone in self.graph.zones.values():
            ...
            # Zone label below
            short = zone.name if len(zone.name) <= 12 else zone.name[:10] + ".."
            ax.annotate(
                short,
                (zone.x, zone.y - 0.55),
                ha="center",
        ...
        legend_items = [
            ...
            mpatches.Patch(
                color=ZONE_COLORS[ZoneType.RESTRICTED.value], label="Restricted"
            ),
            mpatches.Patch(
                color=ZONE_COLORS[ZoneType.PRIORITY.value], label="Priority"
            ),
