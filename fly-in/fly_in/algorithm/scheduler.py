...
- Capacity rules:
    - A zone allows at most max_drones drones simultaneously (start/end unlimited).
...
class Scheduler:
    ...
    def _turn(self, zone_occ: Dict[str, Set[int]]) -> List[str]:
        ...
        # Reservations: zones that will receive an arriving transit drone NEXT turn
        ...
        # (they will arrive THIS turn or next, so reserve NOW for incoming starts)
        ...
        for drone in arriving:
            ...
            # Use raw capacity (no reservations) since this drone IS the reservation
            dest_current = len(zone_occ[dest.name])
            dest_cap = 999999 if (dest.is_end or dest.is_start) else dest.max_drones
