    ...
    def find_shortest_path(
        ...
        while heap:
            ...
            for neighbor, _ in self.graph.get_neighbors(current):
                ...
                # Priority zones have same cost but are preferred via tiebreaking
                ...
                if new_cost < best.get(neighbor.name, 1e18):
                    ...
                    heapq.heappush(
                        heap,
                        (new_cost, counter, neighbor.name, path + [neighbor.name]),
                    )
            ...
            while heap:
                ...
                for neighbor, _ in self.graph.get_neighbors(current):
                    ...
                    if neighbor is not end and neighbor is not start:
                        penalty = zone_usage.get(neighbor.name, 0.0) * penalty_factor
                        move_cost += penalty
