...
class MapParser:
    ...
    def parse_string(self, content: str) -> Graph:
        ...
        for line_num, raw_line in enumerate(lines, start=1):
            ...
            if m:
                if nb_drones_found:
                    raise ParseError("Duplicate nb_drones definition.", line_num)
                value = int(m.group(1))
                if value <= 0:
                    raise ParseError(
                        f"nb_drones must be a positive integer, got {value}.", line_num
                    )
            ...
            if m:
                ...
                if is_start:
                    start_count += 1
                    if start_count > 1:
                        raise ParseError("Multiple start_hub zones defined.", line_num)
                if is_end:
                    end_count += 1
                    if end_count > 1:
                        raise ParseError("Multiple end_hub zones defined.", line_num)
            ...
            if m:
                ...
                if zone_a is None:
                    raise ParseError(
                        f"Connection references undefined zone '{name_a}'.", line_num
                    )
                if zone_b is None:
                    raise ParseError(
                        f"Connection references undefined zone '{name_b}'.", line_num
                    )
    ...
    def _parse_positive_int(
        ...
        try:
            ...
            raise ParseError(
                f"'{field_name}' must be an integer, got '{value_str}'.", line_num
            )
        if value <= 0:
            raise ParseError(
                f"'{field_name}' must be a positive integer, got {value}.", line_num
            )
        return value
