...
class Graph:
    ...
    def add_zone(self, zone: Zone) -> None:
        ...
        if zone.name in self.zones:
            raise ValueError(f"Zone '{zone.name}' already exists in the graph.")
    ...
    def shortest_path(
        ...
        # (cost, zone_name, path)
        heap: List[Tuple[int, str, List[str]]] = [(0, start.name, [start.name])]
    ...
    def find_disjoint_paths(
        ...
        """Find multiple paths with minimal zone overlap using cost-based search.
