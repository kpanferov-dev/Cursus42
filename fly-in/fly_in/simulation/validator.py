...
def validate(graph: Graph, turn_log: List[List[str]]) -> Tuple[bool, str]:
    ...
    drone_pos: Dict[int, str] = {i + 1: graph.start_zone.name for i in range(nb)}
    ...
    for turn_num, tokens in enumerate(turn_log, start=1):
        ...
        for tok in tokens:
            if not tok.startswith("D"):
                return False, f"Turn {turn_num}: token '{tok}' must start with 'D'"
            try:
                dash = tok.index("-")
            except ValueError:
            ...
            if drone_id in seen_drones:
                return False, f"Turn {turn_num}: drone {drone_id} appears twice"
        ...
        for drone_id, target in actions:
            ...
            if in_transit[drone_id]:
                ...
                if target != expected_dest:
                    return False, (
                        f"Turn {turn_num}: drone {drone_id} on link '{current}' "
                        f"must arrive at '{expected_dest}' but moved to '{target}'"
                    )
                ...
                if target in graph.zones:
                    ...
                    if nxt.zone_type == ZoneType.BLOCKED:
                        return False, (
                            f"Turn {turn_num}: cannot enter blocked zone '{target}'"
                        )
                    zone_departures[current] = zone_departures.get(current, 0) + 1
                    zone_arrivals[target] = zone_arrivals.get(target, 0) + 1
                    link_use[conn_zone.name] = link_use.get(conn_zone.name, 0) + 1
                    drone_pos[drone_id] = target
                    if target == graph.end_zone.name:
                        delivered.add(drone_id)
                else:
                    ...
                    if current == conn_link.zone_a.name:
                        ...
                        return False, (
                            f"Turn {turn_num}: drone {drone_id} at '{current}' "
                    ...
                    zone_departures[current] = zone_departures.get(current, 0) + 1
                    link_use[conn_link.name] = link_use.get(conn_link.name, 0) + 1
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python -m fly_in.simulation.validator <map_file>")
        sys.exit(1)
